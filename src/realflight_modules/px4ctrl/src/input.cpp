...
void RC_Data_t::check_validity() {
    if (mode >= -1.1 && mode <= 1.1 && gear >= -1.1 && gear <= 1.1 && reboot_cmd >= -1.1 &&
        ...
        ROS_ERROR(
            "RC data validity check fail. mode=%f, gear=%f, reboot_cmd=%f", mode, gear, reboot_cmd);
    }
}
...
// #define VEL_IN_BODY
#ifdef VEL_IN_BODY /* Set to 1 if the velocity in odom topic is relative to current body frame, \
                      not to world frame.*/
    ...
    static int count = 0;
    if (count++ % 500 == 0) ROS_WARN("VEL_IN_BODY!!!");
#endif
