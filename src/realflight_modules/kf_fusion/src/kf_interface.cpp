...
void KfInterface::receiveVinsInvalidTopic(const std_msgs::BoolConstPtr& vins_invalid_msg) {
    is_vins_invalid_ = vins_invalid_msg->data;
...
void KfInterface::processMeasurements() {
    while (ros::ok() && !ros::isShuttingDown()) {
        ...
        if (cv_msg_.wait_for(
                lock, std::chrono::milliseconds(100), [&]() { return !meas_que_.empty(); })) {
            ...
            for (auto& meas : meas_batch) {
                if (kf_coordiantor_.isInit() == false) {
                    if (state_initializer_.initState(meas) == true) {
                        ...
                        health_monitor_.init(&kf_coordiantor_);
                    ...
                    switch (meas->type) {
                        ...
                        case MeasureType::kOdometry: {
                            ...
                            if (config_.use_motion_capture) {
                                health_monitor_.updateFromMcUpt(odom_meas->timestamp_s);
                            } else {
...
void KfInterface::init(ros::NodeHandle& nh) {
    ...
    pub_kf_fail_  = nh.advertise<std_msgs::Bool>("kf_fail", 10);
    sub_imu_      = nh.subscribe(config_.imu_topic, 1000, &KfInterface::receiveImuTopic, this);
...
void KfInterface::publishImuOdom(double t) {
    Sophus::SO3d state_ori         = kf_coordiantor_.getStateManagerRef().getImuOriState();
    Eigen::Quaterniond state_oir_q = state_ori.unit_quaternion();
    Eigen::Vector3d state_posi     = kf_coordiantor_.getStateManagerRef().getImuTvecState();
    Eigen::Vector3d state_vel      = kf_coordiantor_.getStateManagerRef().getImuVelState();
    Eigen::Vector3d bias_accel     = kf_coordiantor_.getStateManagerRef().getBiasAccelState();
...
void KfInterface::publishExt(double t) {
    Sophus::SO3d state_ext_ori     = kf_coordiantor_.getStateManagerRef().getExtOdomOriState();
    Eigen::Vector3d state_ext_tvec = kf_coordiantor_.getStateManagerRef().getExtOdomTvecState();
