...
bool Zupt::zuptDetection(
    ...
    for (int i = 0; i < parallax_buf_.size(); i++) {
        if (parallax_buf_[i].second > parallax_static_thr_) cnt++;
    }
    double parallax_unstatic_cnt_ratio = (double)cnt / (double)parallax_buf_.size();
    if (parallax_unstatic_cnt_ratio > unstatic_ratio_thr_) is_parallax_static = false;
...
void Zupt::getIMURawAndQuaternion(
...
void Zupt::inputIMU(
    ...
    Eigen::Vector3d linearAcceleration_minus_g =
        linearAcceleration + last_Q.toRotationMatrix().inverse() * Eigen::Vector3d(0, 0, -9.8066);
...
double Zupt::inputFeatureAndCheckParallax(
    double t,
    ...
    const std::map<int, std::vector<std::pair<int, Eigen::Matrix<double, 7, 1>>>>& fs1 =
        featureBuf_[featureBuf_.size() - 2].second;
    const std::map<int, std::vector<std::pair<int, Eigen::Matrix<double, 7, 1>>>>& fs2 =
        featureBuf_[featureBuf_.size() - 1].second;

    int parallax_cnt    = 0;
    double parallax_sum = 0;
    for (auto& feat_in_f2 : fs2) {
        int feat_in_f2_id = feat_in_f2.first;
        auto it = std::find_if(fs1.begin(), fs1.end(), [feat_in_f2_id](const auto& feat_in_f1) {
...
Eigen::Vector3d sin(double w, double t) {
    return Eigen::Vector3d(sin(w * t), sin(w * t), sin(w * t));
}
Eigen::Vector3d cos(double w, double t) {
    return Eigen::Vector3d(cos(w * t), cos(w * t), cos(w * t));
}
...
static void zupt_test() {
    ...
    for (int i = 0, j = 0; i < imu_data_size; i++) {
        ...
        if ((int)(cur_t / static_T_sec) % 2 == 0) {
            ...
            acc_sim_data[i] = imu_data_t(
                cur_t, acc_bias + Ri.inverse() * (sin(2.0 * M_PI * move_freq, cur_t) - gw));
    ...
    fprintf(
        f,
        "# timestamp[s], acc_x[m/s^2], acc_y[m/s^2], acc_z[m/s^2], gyr_x[rad/s^2], gyr_y[rad/s^2], "
        "gyr_z[rad/s^2], vel_norm[m/s], get_img[], is_static[]\n");
    Zupt zupt_obj;
    ...
    for (int i = 0; i < imu_data_size; i++) {
        ...
        zupt_obj.inputIMU(
            acc_sim_data[i].first, acc_sim_data[i].second, gyr_sim_data[i].second,
        ...
        fprintf(
            f, "%f,%f,%f,%f,%f,%f,%f,%f,%d,%d\n", cur_t, acc_sim_data[i].second(0),
            acc_sim_data[i].second(1), acc_sim_data[i].second(2), gyr_sim_data[i].second(0),
            gyr_sim_data[i].second(1), gyr_sim_data[i].second(2), vel_sim_data[i].second.norm(),
            get_img, is_static);
    }
    std::cout << std::endl;
