...
class LowPassFilter
{
  public:
    ...
    void process(const Eigen::Vector3d& acc, const Eigen::Vector3d& gyr,
                 Eigen::Vector3d* acc_filt, Eigen::Vector3d* gyr_filt)
    {
      if (!init_) {
        acc_prev1_ = acc;
        gyr_prev1_ = gyr;
...
{
    ZuptResultInfo() {}
    ZuptResultInfo(double t, bool is_static, 
                   const Eigen::Vector3d& acc_raw,
                   const Eigen::Vector3d& gyr_raw,
                   const Eigen::Quaterniond& q_GI_t)
        : t_(t), is_static_(is_static), acc_raw_(acc_raw),
          gyr_raw_(gyr_raw), q_GI_t_(q_GI_t) {}
...
class Zupt
{
  public:
    ...
    bool zuptDetection(double t, const Eigen::Vector3d* const vel_ptr,
    ...
    double inputFeatureAndCheckParallax(double t, 
            const std::map<int, std::vector<std::pair<int, Eigen::Matrix<double, 7, 1> > > >& features_frame);

    void getIMURawAndQuaternion(double t,
  ...
  private:
    ...
    static constexpr int queue_size_ = static_cast<int>( imu_freq_hz_ / img_freq_hz_ ) * feature_frame_size_;
    ...
    std::deque<std::pair<double, std::map<int, std::vector<std::pair<int, Eigen::Matrix<double, 7, 1> > > > > > featureBuf_;
