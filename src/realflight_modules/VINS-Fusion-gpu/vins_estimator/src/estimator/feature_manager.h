...
class FeaturePerFrame {
 public:
  ...
  FeaturePerFrame(const pair<Eigen::Matrix<double, 64, 1>,
                             Eigen::Matrix<double, 7, 1>> &_point_with_desc,
                  double td) {
    descriptor = _point_with_desc.first;
    point.x() = _point_with_desc.second(0);
    point.y() = _point_with_desc.second(1);
    point.z() = _point_with_desc.second(2);
    uv.x() = _point_with_desc.second(3);
    uv.y() = _point_with_desc.second(4);
    velocity.x() = _point_with_desc.second(5);
    velocity.y() = _point_with_desc.second(6);
    cur_td = td;
    is_stereo = false;
  }
