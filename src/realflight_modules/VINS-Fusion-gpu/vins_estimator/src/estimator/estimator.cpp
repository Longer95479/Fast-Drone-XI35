...
void Estimator::inputIMU(
    double t, const Vector3d &linearAcceleration, const Vector3d &angularVelocity) {
    mBuf.lock();
...
void Estimator::inputFeature(
    double t, const map<int, vector<pair<int, Eigen::Matrix<double, 7, 1>>>> &featureFrame) {
    mBuf.lock();
    featureBuf.push(make_pair(t, featureFrame));
    mBuf.unlock();
        ...
        if (!featureBuf.empty()) {
            ...
            if (USE_IMU && USE_ZUPT) {
                ...
                if (is_first) {
                    ...
                    fprintf(f, "t,is_static,acc_raw_x,acc_raw_y,acc_raw_z,gyr_raw_x,gyr_raw_y,gyr_raw_z,Ba_x,Ba_y,Ba_z,Bg_x,Bg_y,Bg_z,Vx,Vy,Vz,qw,qx,qy,qz,frame_count\n");
                    fclose(f);
                    is_first = false;
                }
                else {
                    FILE* f = fopen("/root/Fast-Drone-XI35/vins_output/zupt_result_log.csv", "a");
                    fprintf(f, "%f,%d,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%d\n",
                                zupt_result_info.t_,
                                zupt_result_info.is_static_,
                                zupt_result_info.acc_raw_(0),
                                zupt_result_info.acc_raw_(1),
                                zupt_result_info.acc_raw_(2),
                                zupt_result_info.gyr_raw_(0),
                                zupt_result_info.gyr_raw_(1),
                                zupt_result_info.gyr_raw_(2),
                                Bas[frame_count](0),
                                Bas[frame_count](1),
                                Bas[frame_count](2),
                                Bgs[frame_count](0),
                                ...
                                Vs[frame_count](1),
                                Vs[frame_count](2),
                                zupt_result_info.q_GI_t_.w(),
                                zupt_result_info.q_GI_t_.x(),
                                zupt_result_info.q_GI_t_.y(),
                                zupt_result_info.q_GI_t_.z(),
                                frame_count);
                    fclose(f);
                }
                ...
                if (is_first_2) {
                    FILE* f = fopen("/root/Fast-Drone-XI35/vins_output/zupt_result_windows.csv", "w");
                    fprintf(f, "t,Zps[0],Zps[1],Zps[2],Zps[3],Zps[4],Zps[5],Zps[6],Zps[7],Zps[8],Zps[9],Zps[10]\n");
                    fclose(f);
                    is_first_2 = false;
                }
                else {
                    FILE* f = fopen("/root/Fast-Drone-XI35/vins_output/zupt_result_windows.csv", "a");
                    if (frame_count == WINDOW_SIZE) {
                        fprintf(f, "%f,", Zps[frame_count].t_);
                        for (int i = 0; i < WINDOW_SIZE + 1; i++) {
                            fprintf(f, "%f", Zps[i].t_);
                            if (i != WINDOW_SIZE) fprintf(f, ",");
                            else fprintf(f, "\n");
                        }
                    }
                    fclose(f);
...
void Estimator::processIMU(
    double t, double dt, const Vector3d &linear_acceleration, const Vector3d &angular_velocity) {
    if (!first_imu) {
        first_imu = true;
    ...
    if (solver_flag == INITIAL) {
        ...
        if (enable_imu_odom_smooth) {
            ...
            if (temp_cur_V_norm > velocity_limit && !have_dropped_one_frame) {
                ROS_WARN(
                    "Curretn velocity exceed limitation! would not update the "
                    "latest states.");
...
bool Estimator::initialStructure() {
    ...
    GlobalSFM sfm;
    if (!sfm.construct(
            frame_count + 1, Q, T, l, relative_R, relative_T, sfm_f, sfm_tracked_points)) {
...
void Estimator::double2vector() {
    ...
    if (USE_IMU) {
        ...
        for (int i = 0; i <= WINDOW_SIZE; i++) {
            ...
            Ps[i] =
                rot_diff * Vector3d(
                               para_Pose[i][0] - para_Pose[0][0], para_Pose[i][1] - para_Pose[0][1],
    ...
    if (USE_IMU) {
        for (int i = 0; i < NUM_OF_CAM; i++) {
            ...
            ric[i] =
                Quaterniond(
                    para_Ex_Pose[i][6], para_Ex_Pose[i][3], para_Ex_Pose[i][4], para_Ex_Pose[i][5])
                    .normalized()
                    .toRotationMatrix();
        }
    }
...
void Estimator::optimization() {
    ...
    if (USE_IMU && USE_ZUPT) {
        for (int i = 0; i < frame_count; i++) {
            ...
            problem.AddResidualBlock(
                zupt_factor, NULL, para_Pose[i], para_SpeedBias[i]);
        }
    }
    ...
    if (enable_ex_prior) {
        ...
        // ExParamPriorFactor(ric[1], tic[1]);
    ...
    for (auto &it_per_id : f_manager.feature) {
        ...
        for (auto &it_per_frame : it_per_id.feature_per_frame) {
            ...
            if (imu_i != imu_j) {
                ...
                problem.AddResidualBlock(
                    f_td, loss_function, para_Pose[imu_i], para_Pose[imu_j], para_Ex_Pose[0],
            ...
            if (STEREO && it_per_frame.is_stereo) {
                ...
                if (imu_i != imu_j) {
                    ProjectionTwoFrameTwoCamFactor *f = new ProjectionTwoFrameTwoCamFactor(
                        ...
                        it_per_frame.cur_td);
                    problem.AddResidualBlock(
                        f, loss_function, para_Pose[imu_i], para_Pose[imu_j], para_Ex_Pose[0],
                    ...
                    ProjectionOneFrameTwoCamFactor *f = new ProjectionOneFrameTwoCamFactor(
                        ...
                        it_per_frame.cur_td);
                    problem.AddResidualBlock(
                        f, loss_function, para_Ex_Pose[0], para_Ex_Pose[1],
                        para_Feature[feature_index], para_Td[0]);
                }
            }
            f_m_cnt++;
    ...
    if (marginalization_flag == MARGIN_OLD) {
        ...
        if (last_marginalization_info &&
            std::count(
                std::begin(last_marginalization_parameter_blocks),
    ...
    for (auto &it_per_id : f_manager.feature) {
        if (it_per_id.estimated_depth > 0) {
            ...
            if ((int)it_per_id.feature_per_frame.size() >= 2 && lastIndex == frame_count) {
                ...
                Vector3d pts_local =
                    nextT.block<3, 3>(0, 0).transpose() * (pts_w - nextT.block<3, 1>(0, 3));
...
double Estimator::reprojectionError(
    Matrix3d &Ri, Vector3d &Pi, Matrix3d &rici, Vector3d &tici, Matrix3d &Rj, Vector3d &Pj,
        ...
        for (auto &it_per_frame : it_per_id.feature_per_frame) {
            ...
            if (imu_i != imu_j) {
                ...
                double tmp_error = reprojectionError(
                    Rs[imu_i], Ps[imu_i], ric[0], tic[0], Rs[imu_j], Ps[imu_j], ric[0], tic[0],
                    depth, pts_i, pts_j);
                err += tmp_error;
                errCnt++;
            ...
            if (STEREO && it_per_frame.is_stereo) {
                ...
                if (imu_i != imu_j) {
                    double tmp_error = reprojectionError(
                        Rs[imu_i], Ps[imu_i], ric[0], tic[0], Rs[imu_j], Ps[imu_j], ric[1], tic[1],
...
void Estimator::fastPredictIMU(
    double t, Eigen::Vector3d linear_acceleration, Eigen::Vector3d angular_velocity) {
...
double Estimator::calCurRepErrAtImuPose(
    int frame_count, const map<int, vector<pair<int, Eigen::Matrix<double, 7, 1>>>> &cur_features) {
    double err    = 0;
    int errCnt    = 0;
    ...
    for (auto &id_pts : cur_features) {
        ...
        err += reprojectionError(
            Rs[imu_i], Ps[imu_i], ric[0], tic[0], Rs[frame_count], Ps[frame_count], ric[0], tic[0],
            depth, pts_i, pts_j);
        errCnt++;
    }
    if (!errCnt) return 0;
...
void Estimator::calCurPoseByPNP(
    ...
    for (auto &id_pts : cur_features) {
        ...
        Vector3d ptsInImu =
            ric[0] * (it->estimated_depth * it->feature_per_frame[0].point) + tic[0];
