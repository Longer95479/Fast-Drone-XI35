...
void Estimator::setParameter() {
  ...
  ProjectionOneFrameTwoCamFactor::sqrt_info =
      FOCAL_LENGTH / 1.5 * Matrix2d::Identity();
  ExParamPriorFactor::sqrt_info =
      ex_prior_sqrt_info * Eigen::Matrix<double, 6, 6>::Identity();
  ExParamPriorFactorOnlyT::sqrt_info =
      ex_prior_sqrt_info * Eigen::Matrix3d::Identity();
  td = TD;
  g = G;
...
void Estimator::inputIMU(double t, const Vector3d &linearAcceleration,
  ...
  if (solver_flag == NON_LINEAR) {
    ...
    pubLatestOdometry(latest_P, latest_Q, latest_V, t,
                      health_monitor.getHealthPercent());
  }
}
...
void Estimator::optimization() {
  ...
  if (enable_ex_prior) {
    ...
    ExParamPriorFactorOnlyT *ex1_prior_factor =
        new ExParamPriorFactorOnlyT(tic[0]);
    ExParamPriorFactorOnlyT *ex2_prior_factor =
        new ExParamPriorFactorOnlyT(tic[1]);
