...
inline double pixelToNormal(double pixel) { return pixel / 389.6706237792969; }
...
void Estimator::setParameter() {
  ...
  StructLineProjectionOneFrameFactor::sqrt_info =
      FOCAL_LENGTH / 1.5 * STRUCT_LINE_SQRT_INFO * Matrix2d::Identity();
  StructLineProjectionTwoFrameFactor::sqrt_info =
      FOCAL_LENGTH / 1.5 * STRUCT_LINE_SQRT_INFO * Matrix2d::Identity();
  HorizonLineProjectionOneFrameFactor::sqrt_info =
      FOCAL_LENGTH / 1.5 * STRUCT_LINE_SQRT_INFO * Matrix2d::Identity();
  HorizonLineProjectionTwoFrameFactor::sqrt_info =
      FOCAL_LENGTH / 1.5 * STRUCT_LINE_SQRT_INFO * Matrix2d::Identity();
  VerticalLineProjectionOneFrameFactor::sqrt_info =
      FOCAL_LENGTH / 1.5 * STRUCT_LINE_SQRT_INFO * Matrix2d::Identity();
  VerticalLineProjectionTwoFrameFactor::sqrt_info =
      FOCAL_LENGTH / 1.5 * STRUCT_LINE_SQRT_INFO * Matrix2d::Identity();
...
void Estimator::inputImage(double t, const cv::Mat &_img,
  ...
  map<int, vector<pair<int, Eigen::Matrix<double, 7, 1>>>> track_feature;
  if (_img1.empty())
    track_feature = featureTracker.trackImage(t, _img);
  else
    track_feature = featureTracker.trackImage(t, _img, _img1);
  for (auto &id_pts : track_feature) {
    featureFrame.first[id_pts.first].emplace_back(
        Eigen::Matrix<double, 64, 1>::Zero(), id_pts.second[0].second);
    if (id_pts.second.size() > 1) {
      featureFrame.first[id_pts.first].emplace_back(
          Eigen::Matrix<double, 64, 1>::Zero(), id_pts.second[1].second);
    }
  }
...
void Estimator::processImage(
  ...
  if (solver_flag == INITIAL) {
    ...
    line_manager.line_triangulate(Rs, Ps, tic, ric);
    // optimization
    if (enable_triang_opti_only) onlyLinesOptimization();
    optimization();
    ...
    if (enable_imu_odom_smooth) {
      ...
      if (temp_cur_V_norm > velocity_limit && !have_dropped_one_frame) {
        ROS_WARN(
            "Curretn velocity exceed limitation! would not update the latest "
            "states.");
...
void Estimator::processImageWithPointsAndStructLines(
    const pair<map<int, vector<pair<Eigen::Matrix<double, 64, 1>,
                                    Eigen::Matrix<double, 7, 1>>>>,
               map<int, pair<vector<uchar>, Eigen::Matrix<double, 8, 1>>>>
        &image,
  ...
  if (solver_flag == INITIAL) {
    ...
    if (STEREO && USE_IMU) {
      ...
      // struct line process
      // add lines
      vector<LineFeatsPtr> tracked_h_lines, new_lines;
      struct_line_manager.addTrackedStructLineAndGetHorizon(
          image.second, td, tracked_h_lines, new_lines);
      // vertical line triangulate
      struct_line_manager.onlyVerticalLineTriangulate(Rs, Ps, tic, ric);
      if (ASSOCIATE_POINTS_TO_LINES) {
        struct_line_manager.onlyVerticalLineTriangulateByPoints(f_manager, Rs,
                                                                Ps, tic, ric);
      }
      // classify the vertical lines among new lines
      vector<LineFeatsPtr> new_vertical_lines, new_other_lines;
      onlyClassifyVerticalLine(new_lines, new_vertical_lines, new_other_lines);
      int new_other_size = new_other_lines.size();
      // RANSAC
      new_other_lines.insert(new_other_lines.end(), tracked_h_lines.begin(),
                             tracked_h_lines.end());
      auto res_ransac = recognizeMHTUsingRANSAC(frame_count, new_other_lines);
      double new_mht =
          res_ransac.first ? res_ransac.second : mht_manager.getLatestMHT();
      // classify the horizon lines among new lines
      vector<LineFeatsPtr> new_horizon_lines;
      new_other_lines.resize(new_other_size);  // only classify the new lines
      onlyClassifyHorizonLine(new_mht, new_other_lines, new_horizon_lines);
      // add to line manager
      struct_line_manager.addNewStrcutLine(frame_count, new_vertical_lines, td);
      struct_line_manager.addNewStrcutLine(frame_count, new_horizon_lines, td);
      // add to mht manager
      mht_manager.insertNewMHT(frame_count, new_mht);
      mht_manager.printMHTWindow();
      // assocaite points to lines
      if (ASSOCIATE_POINTS_TO_LINES) {
        vector<pair<int, Vector4d>> lines_no_trig;
        struct_line_manager.getUninitialLines(image.second, lines_no_trig);
        vector<pair<int, vector<pair<int, double>>>> lines_associa_pts;
        calAssociaPtsForLines(image.first, lines_no_trig, lines_associa_pts);
      ...
      if (frame_count == WINDOW_SIZE) {
        ...
        if (mht_manager.checkMHTWindow()) {
          ...
          struct_line_manager.structLineTriangulate(local_mht, Rs, Ps, tic,
                                                    ric);
          if (ASSOCIATE_POINTS_TO_LINES) {
            struct_line_manager.structLineTriangulateByPoints(
                local_mht, f_manager, Rs, Ps, tic, ric);
          }
    ...
    if (mht_state == HOLD) {
      ...
      if (ASSOCIATE_POINTS_TO_LINES) {
        struct_line_manager.structLineTriangulateByPoints(local_mht, f_manager,
                                                          Rs, Ps, tic, ric);
      }
      ...
      ROS_DEBUG("Strut line rep err changing after optimization: %lf ===> %lf",
                sline_err_bf, sline_err_af);
      ROS_DEBUG("Hold: optimization cost %lf ms.", tic_opt.toc());

      // classify the new lines
      TicToc tic_cn;
      vector<LineFeatsPtr> add_lines;
      bool trigger_mht_detect = structLineClassify(new_lines, add_lines);
      ROS_DEBUG("Hold: structLineClassify cost %lf ms.", tic_cn.toc());
      if (trigger_mht_detect) {
        mht_state = UPDATING;
        mht_manager.clear();
        ROS_WARN("Trigger new mht detection! The old local mht is %lf",
                 local_mht);
      }
      // add to manager
      struct_line_manager.addNewStrcutLine(frame_count, add_lines, td);
    } else {  // updating local mht
      // add lines
      vector<LineFeatsPtr> tracked_h_lines, new_lines;
      struct_line_manager.addTrackedStructLineAndGetHorizon(
          image.second, td, tracked_h_lines, new_lines);
      ...
      if (mht_manager.checkMHTWindow()) {
        ...
        onlyOptimizeMhtAndLines();
        mht_state = HOLD;
        ROS_INFO("Local mht updating finish! The new local mht is %lf",
                 local_mht);
      }
    }
    // assocaite points to lines
    ...
    ROS_DEBUG("Struct line manager has %d line features, %d is triangulated.",
              line_cnt.first, line_cnt.second);
    ...
    ROS_DEBUG(
        "struct line remove outliers counts: %d, remove %d vertical lines, %d "
        "horizon lines.",
        removeIndex.size(), rm_cnt.first, rm_cnt.second);
    ...
    slideWindow();
    f_manager.removeFailures();
    struct_line_manager.removeFailures();
    // prepare output of VINS
    key_poses.clear();
    for (int i = 0; i <= WINDOW_SIZE; i++) key_poses.push_back(Ps[i]);
...
void Estimator::vector2double() {
  ...
  if (USE_STRUCT_LINE) {
    MatrixXd sline_mat =
        struct_line_manager.getLineParamMat(struct_lines_opt_type);
...
void Estimator::double2vector() {
  ...
  // 所以这里的操作过程就相当于是 fix 第一帧的 yaw 和 postion,
  // 使得整个轨迹不会在空间中任意飘。
  ...
  if (USE_IMU) {
    ...
    if (USE_STRUCT_LINE) {
      ...
      if (have_h_lines_opt) {
        ...
        ROS_DEBUG("Local mht changing: %lf ===> %lf ===> %lf", local_mht_last,
                  para_Local_MHT[0][0], local_mht);
      }
      ...
      for (int i = 0; i < line_orth_mat.rows(); i++) {
        ...
        Vector4d line_wo_orth(para_Line[i][0], para_Line[i][1], para_Line[i][2],
                              para_Line[i][3]);
...
void Estimator::optimization() {
  ...
  if (USE_STRUCT_LINE) {
    ceres::LocalParameterization *local_parameterization =
        new MHTParameterization();
    problem.AddParameterBlock(para_Local_MHT[0], SIZE_MHT,
                              local_parameterization);
    if (!ENABLE_MHT_OPT) problem.SetParameterBlockConstant(para_Local_MHT[0]);
    ...
    for (auto &it_per_id : struct_line_manager.struct_line_features) {
      ...
      if (!struct_line_manager.isLineUsable(it_per_id)) continue;
      ...
      // add ParameterBlock
      ceres::LocalParameterization *local_parameterization =
          new StructLineParameterization();
      problem.AddParameterBlock(para_Struct_Line[line_index], SIZE_STRUCT_LINE,
                                local_parameterization);
      ...
      for (auto &it_per_frame : it_per_id.line_feature_per_frame) {
        imu_j++;
        if (imu_j == imu_i) {
          // ceres::CostFunction *struct_line_factor =
          // StructLineProjectionOneFrameFactor::create(it_per_frame.pt_start,
          // it_per_frame.pt_end,
          ...
          //     para_Td[0]);
          if (it_per_id.line_type == VERTICAL) {
            ceres::CostFunction *vertical_line_factor =
                VerticalLineProjectionOneFrameFactor::create(
                    it_per_frame.pt_start, it_per_frame.pt_end);
            problem.AddResidualBlock(vertical_line_factor, loss_function,
                                     para_Struct_Line[line_index],
                                     para_Pose[imu_j], para_Ex_Pose[0]);
          } else {
            ceres::CostFunction *horizon_line_factor =
                HorizonLineProjectionOneFrameFactor::create(
                    it_per_frame.pt_start, it_per_frame.pt_end,
                    it_per_id.line_type);
            problem.AddResidualBlock(horizon_line_factor, loss_function,
                                     para_Struct_Line[line_index],
                                     para_Local_MHT[0], para_Pose[imu_j],
                                     para_Ex_Pose[0]);
          }
        } else {
          // ceres::CostFunction *struct_line_factor =
          // StructLineProjectionTwoFrameFactor::create(it_per_frame.pt_start,
          // it_per_frame.pt_end,
          //                                                                             it_per_frame.velocity_start, it_per_frame.velocity_end,
          //                                                                             it_per_frame.cur_td, it_per_id.line_type);
          // problem.AddResidualBlock(struct_line_factor, loss_function,
          ...
          if (it_per_id.line_type == VERTICAL) {
            ceres::CostFunction *vertical_line_factor =
                VerticalLineProjectionTwoFrameFactor::create(
                    it_per_frame.pt_start, it_per_frame.pt_end);
            problem.AddResidualBlock(vertical_line_factor, loss_function,
                                     para_Struct_Line[line_index],
            ...
            ceres::CostFunction *horizon_line_factor =
                HorizonLineProjectionTwoFrameFactor::create(
                    it_per_frame.pt_start, it_per_frame.pt_end,
                    it_per_id.line_type);
            problem.AddResidualBlock(horizon_line_factor, loss_function,
                                     para_Struct_Line[line_index],
                                     para_Local_MHT[0], para_Pose[imu_i],
                                     para_Pose[imu_j], para_Ex_Pose[0]);
          }
        }
        line_m_cnt++;
    ...
    ROS_DEBUG(
        "line measurements that add to ceres count: %d, line_v_cnt is %d, "
        "line_h_cnt is %d.",
        line_m_cnt, line_v_cnt, line_h_cnt);
    ...
    for (auto &it_per_id : line_manager.line_features) {
      ...
      if (!(it_per_id.used_num >= line_min_obs &&
            ...
            it_per_id.is_triangulated))
        continue;

      line_index++;
      ceres::LocalParameterization *local_parameterization =
          new LineOrthParameterization();
      problem.AddParameterBlock(para_Line[line_index], SIZE_LINE,
                                local_parameterization);

      int imu_i = it_per_id.start_frame, imu_j = imu_i - 1;
      for (auto &it_per_frame : it_per_id.line_feature_per_frame) {
        imu_j++;
        LineProjectionFactor *f = new LineProjectionFactor(
            it_per_frame.pt_start, it_per_frame.pt_end,
            it_per_frame.velocity_start, it_per_frame.velocity_end,
            it_per_frame.cur_td);
        problem.AddResidualBlock(f, loss_function, para_Pose[imu_j],
                                 para_Ex_Pose[0], para_Line[line_index],
                                 para_Td[0]);
        line_m_cnt++;
      }
    }
  ...
  ROS_DEBUG("Struct lines rep err changing at ZERO Spece: %lf ===========> %lf",
            slines_err_bf, slines_err_af);
    ...
    if (USE_STRUCT_LINE && ENABLE_STRUCT_LINE_MARGIN) {
        ...
        for (auto &it_per_frame : it_per_id.line_feature_per_frame) {
          ...
          if (imu_j == imu_i) {
            ...
            if (it_per_id.line_type == VERTICAL) {
              ...
              ResidualBlockInfo *residual_block_info = new ResidualBlockInfo(
                  f, loss_function,
                  vector<double *>{para_Struct_Line[line_index],
                                   para_Pose[imu_i], para_Pose[imu_j],
                                   para_Ex_Pose[0]},
                  vector<int>{0, 1});
              marginalization_info->addResidualBlockInfo(residual_block_info);
      ...
      for (auto &it_per_id : line_manager.line_features) {
        ...
        for (auto &it_per_frame : it_per_id.line_feature_per_frame) {
          ...
          ResidualBlockInfo *residual_block_info = new ResidualBlockInfo(
              f, loss_function,
              vector<double *>{para_Pose[imu_j], para_Ex_Pose[0],
                               para_Line[line_index], para_Td[0]},
              vector<int>{2});
          marginalization_info->addResidualBlockInfo(residual_block_info);
        }
      }
...
void Estimator::onlyLinesOptimization() {
  ...
  for (auto &it_per_id : line_manager.line_features) {
    it_per_id.used_num = it_per_id.line_feature_per_frame.size();
    if (!(it_per_id.used_num >= line_min_obs &&
          it_per_id.start_frame < WINDOW_SIZE - 2 && it_per_id.is_triangulated))
      continue;

    feature_index++;
    ceres::LocalParameterization *local_parameterization =
        new LineOrthParameterization();
    problem.AddParameterBlock(para_Line[feature_index], SIZE_LINE,
                              local_parameterization);
    ...
    for (auto &it_per_frame : it_per_id.line_feature_per_frame) {
      ...
      LineProjectionFactor *f = new LineProjectionFactor(
          ...
          it_per_frame.cur_td);
      problem.AddResidualBlock(f, loss_function, para_Pose[imu_j],
                               para_Ex_Pose[0], para_Line[feature_index],
                               para_Td[0]);
    }
  }

  if (feature_index < 3) return;
  TicToc tic_ol;
  ceres::Solver::Options options;
  options.linear_solver_type = ceres::DENSE_SCHUR;
  options.max_num_iterations = NUM_ITERATIONS;
  ...
  MatrixXd line_orth_mat(line_manager.getFeatureCount(), 4);
  for (int i = 0; i < line_orth_mat.rows(); i++) {
    Vector4d orth(para_Line[i][0], para_Line[i][1], para_Line[i][2],
                  para_Line[i][3]);
    line_orth_mat.row(i) = orth;
  }
  line_manager.setLineFeature(line_orth_mat);
...
void Estimator::onlyOptimizeMhtAndLines() {
  ...
  for (auto &it_per_id : struct_line_manager.struct_line_features) {
    ...
    ceres::LocalParameterization *local_parameterization =
        new StructLineParameterization();
    problem.AddParameterBlock(para_Struct_Line[feature_index], SIZE_STRUCT_LINE,
                              local_parameterization);
    // add Residual
    int imu_i = it_per_id.start_frame, imu_j = imu_i - 1;
    for (auto &it_per_frame : it_per_id.line_feature_per_frame) {
      ...
      if (imu_j == imu_i) {
        ceres::CostFunction *struct_line_factor =
            StructLineProjectionOneFrameFactor::create(
                it_per_frame.pt_start, it_per_frame.pt_end,
                it_per_frame.velocity_start, it_per_frame.velocity_end,
                it_per_frame.cur_td, it_per_id.line_type);
        problem.AddResidualBlock(
            struct_line_factor, loss_function, para_Struct_Line[feature_index],
            para_Local_MHT[0], para_Pose[imu_j], para_Ex_Pose[0], para_Td[0]);
      } else {
        ceres::CostFunction *struct_line_factor =
            StructLineProjectionTwoFrameFactor::create(
                it_per_frame.pt_start, it_per_frame.pt_end,
                it_per_frame.velocity_start, it_per_frame.velocity_end,
                it_per_frame.cur_td, it_per_id.line_type);
        problem.AddResidualBlock(struct_line_factor, loss_function,
...
void Estimator::slideWindowNew() {
  ...
  if (USE_STRUCT_LINE) {
    ...
    if (mht_state == UPDATING) mht_manager.slideMHTWindowNew();
  } else
...
void Estimator::slideWindowOld() {
  ...
  if (shift_depth) {
    ...
    if (USE_STRUCT_LINE) struct_line_manager.removeBackShiftParam(P0, P1);
  } else
  ...
  if (!USE_STRUCT_LINE)
    ...
    if (mht_state == UPDATING) mht_manager.slideMHTWindowOld();
  }
}
...
//两个端点到线的平均距离
double Estimator::lineReprojectionError(const Matrix3d &Ri, const Vector3d &Pi,
                                        const Vector3d &pt_start,
                                        const Vector3d &pt_end,
...
void Estimator::lineOutliersRejection(set<int> &removeIndex) {
  ...
  for (auto &it_per_id : line_manager.line_features) {
    ...
    for (auto &it_per_frame : it_per_id.line_feature_per_frame) {
      ...
      double err = lineReprojectionError(Rwc, twc, it_per_frame.pt_start,
                                         it_per_frame.pt_end, line_w_pluk);
...
void Estimator::structLineOutliersRejection(set<int> &removeIndex) {
  ...
  for (auto &it_per_id : struct_line_manager.struct_line_features) {
    ...
    int feature_id = it_per_id.feature_id;
    int imu_i = it_per_id.start_frame, imu_j = imu_i - 1;
    Vector6d line_w_pluk =
        it_per_id.getPlukInWorldFromParam(local_mht, Rs, Ps, tic, ric);
    double max_err = 0;
    for (auto &it_per_frame : it_per_id.line_feature_per_frame) {
      imu_j++;
      Matrix3d Rwc = Rs[imu_j] * ric[0];
...
double Estimator::calAllStructLinesReprojectionErrorAtZeroSpace() {
  ...
  Vector3d tic_temp[2];
  for (int i = 0; i < WINDOW_SIZE + 1; i++) {
    Rs_temp[i] = Quaterniond(para_Pose[i][6], para_Pose[i][3], para_Pose[i][4],
                             para_Pose[i][5])
                     .normalized()
                     .toRotationMatrix();
    Ps_temp[i] = Vector3d(para_Pose[i][0], para_Pose[i][1], para_Pose[i][2]);
  }
  ric_temp[0] = Quaterniond(para_Ex_Pose[0][6], para_Ex_Pose[0][3],
                            para_Ex_Pose[0][4], para_Ex_Pose[0][5])
                    .normalized()
                    .toRotationMatrix();
  tic_temp[0] =
      Vector3d(para_Ex_Pose[0][0], para_Ex_Pose[0][1], para_Ex_Pose[0][2]);
  ...
  for (auto &it_per_id : struct_line_manager.struct_line_features) {
    ...
    int feature_id = it_per_id.feature_id;
    int imu_i = it_per_id.start_frame, imu_j = imu_i - 1;
    Vector6d line_w_pluk = it_per_id.getPlukInWorldFromParam(
        local_mht_temp, Rs_temp, Ps_temp, tic_temp, ric_temp);
    double max_err = 0;
    for (auto &it_per_frame : it_per_id.line_feature_per_frame) {
      imu_j++;
...
//计算imu预测位姿下的平均重投影误差
double Estimator::calCurRepErrAtImuPose(
    int frame_count,
  ...
  for (auto &id_pts : cur_features) {
    ...
    auto it = find_if(feature.begin(), feature.end(),
                      [feature_id](const FeaturePerId &id_f) {
    ...
    err +=
        reprojectionError(Rs[imu_i], Ps[imu_i], ric[0], tic[0], Rs[frame_count],
                          Ps[frame_count], ric[0], tic[0], depth, pts_i, pts_j);
    errCnt++;
  }
  if (!errCnt) return 0;
...
//计算最新帧的PNP位姿
void Estimator::calCurPoseByPNP(
    int frame_count,
    const map<int, vector<pair<int, Eigen::Matrix<double, 7, 1>>>>
        &cur_features,
  ...
  for (auto &id_pts : cur_features) {
    ...
    Vector3d ptsInImu =
        ric[0] * (it->estimated_depth * it->feature_per_frame[0].point) +
        tic[0];
...
pair<bool, double> Estimator::getTwoLineSimScore(const Vector4d &line0,
  ...
  if (diff_ang < LINE_SIM_ANGLE_THRESH && diff_dist < LINE_SIM_DIST_THRESH) {
    res.first = true;
    res.second =
        (fabs(diff_ang - LINE_SIM_ANGLE_THRESH) / LINE_SIM_ANGLE_THRESH +
         fabs(diff_dist - LINE_SIM_DIST_THRESH) / LINE_SIM_DIST_THRESH) /
        2;
  }
  return res;
}
...
bool Estimator::structLineClassify(const vector<LineFeatsPtr> &new_lines,
  ...
  // classfy the line
  for (auto &line_ptr : new_lines) {
    vector<pair<int, double>> vec_score;
    Vector4d line_se = line_ptr->line_se;
    Vector2d mid_p = (line_se.head(2) + line_se.tail(2)) / 2;
    ...
    if (sim_z.first) vec_score.emplace_back(0, sim_z.second);
    if (sim_x.first) vec_score.emplace_back(1, sim_x.second);
    if (sim_y.first) vec_score.emplace_back(2, sim_y.second);
    ...
    if (!vec_score.empty()) {
      ...
      line_ptr->setLineType(cur_line_type);
      out_lines.push_back(line_ptr);
    }
  }
  int input_size = new_lines.size();
  int output_size = out_lines.size();
  ROS_DEBUG(
      "structLineClassify: new lines input size %d, classified successfully "
      "counts %d, vertical counts is %d, horizon counts is %d.",
      input_size, output_size, vertical_counts, horizon_counts);
...
//只划分垂直线条，在局部曼哈顿更新时调用
void Estimator::onlyClassifyVerticalLine(const vector<LineFeatsPtr> &new_lines,
  ...
  for (auto &line_ptr : new_lines) {
    ...
    auto sim_z = getTwoLineSimScore(line_mvz, line_se);
    if (sim_z.first) {
      line_ptr->setLineType(VERTICAL);
      vertical_lines.push_back(line_ptr);
    } else
      other_lines.push_back(line_ptr);
  }
  ROS_DEBUG(
      "onlyClassifyVerticalLine: Input %d new lines, classified %d vertical "
      "lines, %d other lines",
      new_lines.size(), vertical_lines.size(), other_lines.size());
}
//给定局部曼哈顿对水平线条进行划分
void Estimator::onlyClassifyHorizonLine(
    double ransac_local_mht, const vector<LineFeatsPtr> &h_lines_candi,
  ...
  Vector2d vp_x = getVpFromDDs(DD_x);
  Vector2d vp_y = getVpFromDDs(DD_y);
  for (auto &line_ptr : h_lines_candi) {
    vector<pair<int, double>> vec_score;
    Vector4d line_se = line_ptr->line_se;
    Vector2d mid_p = (line_se.head(2) + line_se.tail(2)) / 2;
    ...
    if (!vec_score.empty()) {
      ...
      line_ptr->setLineType(cur_line_type);
      h_lines.push_back(line_ptr);
    }
  }
  ROS_DEBUG(
      "onlyClassifyHorizonLine: Input %d lines, classified %d horizon lines at "
      "the new mht:%lf",
      h_lines_candi.size(), h_lines.size(), ransac_local_mht);
}
//
int Estimator::countNumForHorizonClassify(
    const vector<LineFeatsPtr> &cur_lines_all, const Vector2d &vp_x,
    const Vector2d &vp_y) {
  int counts = 0;
  for (auto &line_ptr : cur_lines_all) {
    Vector4d line_se = line_ptr->line_se;
    Vector2d mid_p = (line_se.head(2) + line_se.tail(2)) / 2;
    Vector4d line_mvx, line_mvy;
    line_mvx << mid_p, vp_x;
    ...
    auto sim_x = getTwoLineSimScore(line_mvx, line_se);
    auto sim_y = getTwoLineSimScore(line_mvy, line_se);
    if (sim_x.first || sim_y.first) counts++;
  }
  return counts;
}
// RANSAC计算新MHT，输入cur_lines_all需要已剔除垂直线
pair<bool, double> Estimator::recognizeMHTUsingRANSAC(
    int frame_count, const vector<LineFeatsPtr> &cur_lines_all) {
  if (cur_lines_all.empty()) return pair<bool, double>(false, 0);
  ...
  ROS_DEBUG("recognizeMHTUsingRANSAC: l_v is (%lf, %lf, %lf)", l_v(0), l_v(1),
            l_v(2));
  // gen rand
  ...
  while (iteration_counts--) {
    ...
    int select_index =
        static_cast<int>(std::floor(random_number * cur_lines_all.size()));
    Vector4d cur_line = cur_lines_all[select_index]->line_se;
    ROS_DEBUG(
        "recognizeMHTUsingRANSAC: select_index is %d, cur_line is (%lf, %lf, "
        "%lf, %lf)",
        select_index, cur_line(0), cur_line(1), cur_line(2), cur_line(3));
    // cal vpx_w and vpy_w
    Vector3d l_cur = getLineExpression(cur_line);
    ROS_DEBUG("recognizeMHTUsingRANSAC: l_cur is (%lf, %lf, %lf)", l_cur(0),
              l_cur(1), l_cur(2));
    ...
    ROS_DEBUG(
        "recognizeMHTUsingRANSAC: vp_l_its_e_1 is %lf, vp_l_its_e_2 is %lf",
        vp_l_its_e_1, vp_l_its_e_2);

    if (std::isinf(vpx_l(0)) || std::isinf(vpx_l(1)) || std::isnan(vpx_l(0)) ||
        std::isnan(vpx_l(1)))
      continue;
    Vector3d vpx_n;
    vpx_n << vpx_l, 1.0;
    ...
    ROS_DEBUG(
        "recognizeMHTUsingRANSAC: current vanish point is (%lf, %lf), vpx_n is "
        "(%lf, %lf, %lf), vpx_w is (%lf, %lf, %lf)",
        vpx_l(0), vpx_l(1), vpx_n(0), vpx_n(1), vpx_n(2), vpx_w(0), vpx_w(1),
        vpx_w(2));
    vpx_w(2) = 0;
    ...
    ROS_DEBUG(
        "recognizeMHTUsingRANSAC: vanish point after normalized is :vpx_r(%lf, "
        "%lf), vpy_r(%lf, %lf)",
        vpx_r(0), vpx_r(1), vpy_r(0), vpy_r(1));
    // cal counts for curretn vp
    int cur_counts = countNumForHorizonClassify(cur_lines_all, vpx_r, vpy_r);
    ROS_DEBUG(
        "recognizeMHTUsingRANSAC: current counts based on new normalized vp is "
        "%d",
        cur_counts);
  ...
  ROS_DEBUG(
      "MHT-RANSAC: the optimal_mht is %lf[deg], optimal_vpx is (%f, %f, %f).",
      optimal_mht / M_PI * 180.0, optimal_vpx(0), optimal_vpx(1),
      optimal_vpx(2));
  return res;
}
//将vpx_w限制在第一象限
...
Vector2d Estimator::getAdaptiveVp() {
  ...
  else if (relative_ang_recog >= 3 * M_PI / 4 ||
           relative_ang_recog < -3 * M_PI / 4)
    dominant_ang_recog = normalizeAngle(local_mht + M_PI);
  else
    dominant_ang_recog = normalizeAngle(local_mht - M_PI / 2);
  Matrix3d R_wc = Rs[frame_count] * ric[0];
  Vector3d vpx_w =
      Vector3d(cos(dominant_ang_recog), sin(dominant_ang_recog), 0);
  Vector3d vpx_c = R_wc.transpose() * vpx_w;
...
pair<double, Vector2d> Estimator::getAdaptiveDDs() {
  ...
  if (mht_state == UPDATING) return res;

  double cur_gmht = gmht_manager.matchGlobalLMHT(local_mht);
  if (cur_gmht == -1) return res;
...
void Estimator::calAssociaPtsForLines(
  ...
  for (auto &line : lines) {
    ...
    for (int i = 0; i < pts_l_dist.rows(); i++) {
      if (pts_l_dist(i) > pixelToNormal(3)) continue;

      double px = cur_pts_mat(i, 0);
      double py = cur_pts_mat(i, 1);
      if (px < min_lx - pixelToNormal(3) || px > max_lx + pixelToNormal(3) ||
          py < min_ly - pixelToNormal(3) || py > max_ly + pixelToNormal(3))
        continue;
      ...
      if (side1 <= pixelToNormal(9) || side2 <= pixelToNormal(9) ||
          ((side1 < line_side + side2) && (side2 < line_side + side1)))
        pt_id_dist.emplace_back(pts_id[i], pts_l_dist[i]);
    }
    associa_pts.emplace_back(line.first, pt_id_dist);
    // ROS_DEBUG("calAssociaPtsForLines: line-%d associate %d points",
    // line.first, pt_id_dist.size());
  }
}
...
void Estimator::DrawImage(double cur_header) {
  cv::Mat cur_img;
  {
    ...
    while (!img0_buf.empty() &&
           img0_buf.front()->header.stamp.toSec() < cur_header) {
      img0_buf.pop();
    }
    if (img0_buf.empty()) return;
  ...
  int fontFace =
      cv::FONT_HERSHEY_COMPLEX_SMALL;  // 字体类型 FONT_HERSHEY_COMPLEX_SMALL
  ...
  int baseline;
  cv::Size text1Size =
      cv::getTextSize(text1, fontFace, fontScale, thickness, &baseline);
  ...
  // cv::Size text2Size = cv::getTextSize(text2, fontFace, fontScale, thickness,
  // &baseline); int x2 = cur_img.cols - text2Size.width - margin; int y2 = y1 +
  // text1Size.height + 5; cv::Point textOrg2(x2, y2); cv::putText(cur_img,
  // text2, textOrg2, fontFace, fontScale, color, thickness);
  ...
  if (mht_state == HOLD) {
    ...
    cv::arrowedLine(cur_img, st, ed, cv::Scalar(0, 255, 0), 3, cv::LINE_AA, 0,
                    0.1);
  }
...
void Estimator::DrawImage_v2(double cur_header) {
  ...
  cv::Size text2Size =
      cv::getTextSize(text2, fontFace, fontScale, thickness, &baseline);
  ...
  // cv::Size text3Size = cv::getTextSize(text3, fontFace, fontScale, thickness,
  // &baseline); int x3 = cur_img.cols - text3Size.width - margin; int y3 = y2 +
  // text3Size.height + 5; cv::Point textOrg3(x3, y3); cv::putText(cur_img,
  // text3, textOrg3, fontFace, fontScale, color, thickness);
