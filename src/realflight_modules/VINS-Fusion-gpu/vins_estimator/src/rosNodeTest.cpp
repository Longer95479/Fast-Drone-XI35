...
// adjust the line<ps_x, ps_y, pe_x, pe_y, ps_velo_x, ps_velo_y, pe_velo_x,
// pe_velo_y>
void feature_callback(const sensor_msgs::PointCloudConstPtr &feature_msg) {
  ...
  for (unsigned int i = 0; i < feature_msg->points.size(); i++) {
    if (feature_msg->channels.size() > 7 &&
        feature_msg->channels[7].values[i] == 1) {  // line features
      ...
      pair<vector<uchar>, Eigen::Matrix<double, 8, 1> > line_feats;
      Eigen::Matrix<double, 8, 1> line_start_end;
      line_start_end << start_x, start_y, end_x, end_y, start_x_velocity,
          start_y_velocity, end_x_velocity, end_y_velocity;
      line_feats.second = line_start_end;
      auto &line_desc = line_feats.first;
      line_desc.resize(32);
      for (int j = 0; j < 32; j++) {
        line_desc[j] = feature_msg->channels[8 + j].values[i];
      }
      featureFrame.second[feature_id] = line_feats;
    } else {  // points
      int feature_id = feature_msg->channels[0].values[i];
      ...
      ROS_ASSERT(z == 1);
      pair<Eigen::Matrix<double, 64, 1>, Eigen::Matrix<double, 7, 1> >
          point_feats;
      point_feats.second << x, y, z, p_u, p_v, velocity_x, velocity_y;
      if (feature_msg->channels.size() > 8) {  // has descriptor channel
        for (int j = 0; j < 64; j++) {
          point_feats.first(j, 0) = feature_msg->channels[8 + j].values[i];
        }
      }
      featureFrame.first[feature_id].push_back(point_feats);
    }
  }
...
int main(int argc, char **argv) {
  ...
  ros::Subscriber sub_img_frontend = n.subscribe<sensor_msgs::Image>(
      "/feature_tracker/line_img", 2000,
      std::bind(&Estimator::imgCallback, &estimator, std::placeholders::_1));
