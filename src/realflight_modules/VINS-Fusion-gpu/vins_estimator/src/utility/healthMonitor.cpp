...
void HealthMonitor::updateFromVisUpt(double ts) {
  ...
  ROS_WARN(
      "HealthMonitor: update from visual, cur_feat_num is %d, increment is %f.",
      cur_feat_num, increment);
  mtx_val.lock();
...
bool HealthMonitor::isVINSHealthy() {
  ...
  if (estimator_ptr_->Bas[WINDOW_SIZE].norm() > 2.5) {
    ROS_WARN("HealthMonitor: big IMU acc bias estimation %f.",
             estimator_ptr_->Bas[WINDOW_SIZE].norm());
    return false;
  }
  if (estimator_ptr_->Bgs[WINDOW_SIZE].norm() > 1.0) {
    ROS_WARN("HealthMonitor: big IMU gyro bias estimation %f.",
             estimator_ptr_->Bgs[WINDOW_SIZE].norm());
    return false;
  }
  if (estimator_ptr_->Ps[WINDOW_SIZE].norm() > maxTranslation) {
    ROS_WARN("HealthMonitor: big position estimation %f.",
             estimator_ptr_->Ps[WINDOW_SIZE].norm());
    return false;
  }
  if (estimator_ptr_->Vs[WINDOW_SIZE].norm() > maxVelocity) {
    ROS_WARN("HealthMonitor: big velocity estimation %f.",
             estimator_ptr_->Vs[WINDOW_SIZE].norm());
    return false;
  }
  return true;
}