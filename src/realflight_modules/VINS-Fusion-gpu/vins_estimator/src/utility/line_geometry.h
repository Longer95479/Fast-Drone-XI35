...
template <typename T>
Eigen::Matrix<T, 6, 1> plukTransformPoseTemp(
    const Eigen::Matrix<T, 6, 1> &line_in, const Eigen::Matrix<T, 3, 3> &R,
...
Vector4d pointsToPlane(const Vector3d &p1, const Vector3d &p2,
                       const Vector3d &p3);
Vector6d planesToLine(const Vector4d &pi1, const Vector4d &pi2);
Matrix3d skewSymmetric(const Vector3d &v);
Vector6d plukTransformPose(const Vector6d &line_in, const Matrix3d &R,
                           const Vector3d &t);
