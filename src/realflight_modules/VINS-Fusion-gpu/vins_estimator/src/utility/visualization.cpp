...
void registerPub(ros::NodeHandle &n) {
    ...
    pub_vins_fail = n.advertise<std_msgs::Bool>("vins_fail", 1000);
    ...
    if (estimator.solver_flag == Estimator::SolverFlag::NON_LINEAR) {
        ...
        if (STEREO) {
            ...
            if (PUB_RECTIFY) {
                ...
                Vector3d R_P_r = P_r;
                Quaterniond R_R_l =
                    Quaterniond(estimator.Rs[i] * estimator.ric[0] * rectify_R_left.inverse());
                Quaterniond R_R_r =
                    Quaterniond(estimator.Rs[i] * estimator.ric[1] * rectify_R_right.inverse());
