...
void PoseGraph::addKeyFrame(KeyFramePtr cur_kf, bool flag_detect_loop) {
  ...
  if (flag_detect_loop) {
    TicToc tmp_t;
    loop_index = detectLoopByVLAD(cur_kf);
  } else {
    addKeyFrameIntoVoc(cur_kf);
  }
  if (loop_index != -1) {
    // printf(" %d detect loop with %d \n", cur_kf->index, loop_index);
    ROS_WARN("%d detect loop closure, the loop index is %d.", cur_kf->index,
             loop_index);
...
int PoseGraph::detectLoopByVLAD(KeyFramePtr keyframe) {
  cv::Mat compressed_image;
  if (DEBUG_IMAGE) {
    int feature_num = keyframe->point_2d_norm.size();
    cv::resize(keyframe->image, compressed_image, cv::Size(376, 240));
    putText(compressed_image,
            "feature_num:" + to_string(feature_num) +
                " index:" + to_string(keyframe->index),
            cv::Point2f(10, 10), cv::FONT_HERSHEY_SIMPLEX, 0.4,
            cv::Scalar(255));
    image_pool[keyframe->index] = compressed_image;
  }
  // find the earliest loop candi
  int loop_index = -1;
  double similarity = -1;
  if (keyframe->index <= 50) {
    return loop_index;
  }
  for (auto it = keyframelist.begin(); it != keyframelist.end(); it++) {
    if ((*it)->index >= keyframe->index - 50) {
      break;
    }
    similarity = (*it)->vlad_desc.transpose() * keyframe->vlad_desc;
    // ROS_INFO("detectLoopByVLAD: frame_%d similar to frame_%d with %lf.",
    //          keyframe->index, (*it)->index, similarity);
    if (similarity > 0.85) {
      loop_index = (*it)->index;
      break;
    }
  ...
  if (loop_index != -1) {
    ...
    if (DEBUG_IMAGE) {
      loop_result = compressed_image.clone();
      auto it = image_pool.find(loop_index);
      cv::Mat loop_image = (it->second).clone();
      putText(loop_image, "loop score:" + to_string(similarity),
              cv::Point2f(10, 50), cv::FONT_HERSHEY_SIMPLEX, 0.4,
              cv::Scalar(255));
      cv::hconcat(loop_result, loop_image, loop_result);
      cvtColor(loop_result, loop_result, cv::COLOR_GRAY2RGB);
      sensor_msgs::ImagePtr msg =
          cv_bridge::CvImage(std_msgs::Header(), "bgr8", loop_result)
              .toImageMsg();
      msg->header.stamp = ros::Time(keyframe->time_stamp);
      pub_loop_img.publish(msg);
    }
  }
  return loop_index;
}
...
void PoseGraph::loadPoseGraph() {
  ...
  if (pFile == NULL) {
    printf(
        "lode previous pose graph error: wrong previous pose graph path or "
        "no "
  ...
  while (fscanf(pFile,
    ...
    VIO_Ty, VIO_Tz, PG_Tx, PG_Ty, PG_Tz, VIO_Qw, VIO_Qx, VIO_Qy, VIO_Qz,
    PG_Qw, PG_Qx, PG_Qy, PG_Qz, loop_index, loop_info_0, loop_info_1,
    loop_info_2, loop_info_3, loop_info_4, loop_info_5, loop_info_6,
    loop_info_7, keypoints_num);
    */
    cv::Mat image;
    ...
    KeyFramePtr keyframe = std::shared_ptr<KeyFrame>(new KeyFrame(
