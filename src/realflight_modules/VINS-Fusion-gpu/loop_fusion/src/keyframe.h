...
#define MIN_LOOP_NUM 10  // 25

using namespace Eigen;
using namespace std;
using namespace DVision;

using XDescType = Eigen::Matrix<double, 64, 1>;
using VladType = Eigen::Matrix<double, 4096, 1>;
...
class KeyFrame {
 public:
  static std::shared_ptr<KeyFrame> create(
      double _time_stamp, int _index, Vector3d &_vio_T_w_i,
      Matrix3d &_vio_R_w_i, cv::Mat &_image, vector<cv::Point3f> &_point_3d,
  ...
  bool findConnection(KeyFramePtr old_kf);
  void computeWindowBRIEFPoint();
  void computeBRIEFPoint();
  void setXFeatsPoints(vector<cv::Point2f> &points_uv,
  ...
  bool searchInAera(const XDescType &tgt_xdesc,
  ...
  void searchByBRIEFDes(std::vector<cv::Point2f> &matched_2d_old,
                        ...
                        const std::vector<cv::KeyPoint> &keypoints_old,
                        const std::vector<cv::KeyPoint> &keypoints_old_norm);
  void searchByXFeatDes(std::vector<cv::Point2f> &matched_2d_old,
                        std::vector<cv::Point2f> &matched_2d_old_norm,
                        std::vector<uchar> &status,
                        const std::vector<XDescType> &descriptors_old,
                        const std::vector<cv::KeyPoint> &keypoints_old,
                        const std::vector<cv::KeyPoint> &keypoints_old_norm);
  void searchByXFeatDes(KeyFramePtr old_kf,
 ...
 private:
  cv::Mat convertDescriptorsToMat(const std::vector<XDescType> &descriptors);
  std::vector<cv::DMatch> matchWithFlann(const cv::Mat &desc1,
                                         const cv::Mat &desc2,
                                         float ratio_thresh = 0.7f,
                                         float min_similarity = 0.7f);
  std::vector<cv::DMatch> bruteForceMatch(const cv::Mat &desc1,
                                          const cv::Mat &desc2,
                                          float min_similarity = 0.7f);
 ...
 public:
  ...
  vector<XDescType> keypoints_xdesc;
