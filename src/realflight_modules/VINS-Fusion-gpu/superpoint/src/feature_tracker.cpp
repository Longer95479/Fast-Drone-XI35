...
//特征点缩放：原始图像大小->网络输入大小
vector<cv::Point2f> FeatureTracker::pts_ori_to_resized(
    const vector<cv::Point2f> &ori_pts) {
  vector<cv::Point2f> resized_pts;
  double w_scale =
      feature_tracker_config.resized_width / (double)feature_tracker_config.col;
  double h_scale = feature_tracker_config.resized_height /
                   (double)feature_tracker_config.row;
  for (auto &pt : ori_pts) {
...
//特征点缩放：网络输入大小->原始图像大小
vector<cv::Point2f> FeatureTracker::pts_resized_to_ori(
    const vector<cv::Point2f> &resized_pts) {
  vector<cv::Point2f> ori_pts;
  double w_scale =
      (double)feature_tracker_config.col / feature_tracker_config.resized_width;
  double h_scale = (double)feature_tracker_config.row /
                   feature_tracker_config.resized_height;
...
void FeatureTracker::extractKeyPoints(vector<cv::Point2f> &new_pts,
                                      float response_thresh, float nms_range) {
  new_pts.clear();
  ...
  for (int i = 0; i < heat_map_size; ++i) {
    if (*(cur_heatmap + i) < response_thresh) continue;
  ...
  auto id_pts_reserved = nms_process(kpts, indexes, nms_range);
...
void FeatureTracker::extractSquareROIPtsDesc(const cv::Point2f &ori_pt,
                                             ...
                                             aligned_vector<DescV> &descs) {
  if (cur_heatmap == nullptr || cur_desc == nullptr) return;
  int w = feature_tracker_config.resized_width;
  int h = feature_tracker_config.resized_height;
  // scale cof
  double w_scale =
  ...
  for (int i = ori_x - half_len; i <= ori_x + half_len; i++) {
    for (int j = ori_y - half_len; j <= ori_y + half_len; j++) {
      ...
      if (cur_heatmap[j * w + i] > track_assist_args.roi_pts_threshold) {
        ...
        scores.push_back(cur_heatmap[j * w + i]);
      }
    }
  }
...
void FeatureTracker::track_img_use_opticalflow(double _cur_time,
  ...
  if (n_max_cnt > 0) {  // extract features
    ...
    if (feature_tracker_config
      ...
      extractKeyPoints(new_pts_v, feature_tracker_config.new_kpts_threshold,
                       feature_tracker_config.nms_threshold);
  ...
  // // update descriptors
  // if (feature_tracker_config.use_retrack) {  // scale
...
void FeatureTracker::etractLoopFeatsAndPub(ros::Publisher &pub_msg) {
  loop_local_pts.clear();
  loop_local_un_pts.clear();
  loop_local_xdesc.clear();
  extractKeyPoints(loop_local_pts, feature_tracker_config.new_kpts_threshold,
                   20);
  extractDescriptors(loop_local_pts, loop_local_xdesc);
  loop_local_un_pts = undistortedPts(loop_local_pts, m_camera[0]);

  sensor_msgs::PointCloudPtr feature_points(new sensor_msgs::PointCloud);
  sensor_msgs::ChannelFloat32 ch_u;
  sensor_msgs::ChannelFloat32 ch_v;
  sensor_msgs::ChannelFloat32 ch_desc[64];
  feature_points->header.stamp = ros::Time(cur_time);
  feature_points->header.frame_id = "world";
  for (size_t i = 0; i < loop_local_pts.size(); i++) {
    geometry_msgs::Point32 p;
    p.x = loop_local_un_pts[i].x;
    p.y = loop_local_un_pts[i].y;
    p.z = 1;
    feature_points->points.push_back(p);
    ch_u.values.push_back(loop_local_pts[i].x);
    ch_v.values.push_back(loop_local_pts[i].y);
    for (int j = 0; j < 64; j++) {
      ch_desc[j].values.push_back(loop_local_xdesc[i](j, 0));
    }
  }
  feature_points->channels.push_back(ch_u);
  feature_points->channels.push_back(ch_v);
  for (int j = 0; j < 64; j++) {
    feature_points->channels.push_back(ch_desc[j]);
  }
  pub_msg.publish(feature_points);
}