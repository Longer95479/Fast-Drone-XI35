...
bool Xfeat::build() {
  ...
  std::cout << "deserialize xfeat engine failed, will build it at runtime"
            << std::endl;
...
bool Xfeat::construct_network(
  ...
  auto parsed =
      parser->parseFromFile(xfeat_config_.onnx_file.c_str(),
...
bool Xfeat::infer(const cv::Mat &image_,
  ...
  cv::Mat image;
  cv::resize(image_, image, cv::Size(resized_width, resized_height), 0.0, 0.0,
             cv::INTER_AREA);
  ...
  // ROS_DEBUG("xp: process input(image to host mem) cost %f ms.",
  // tic_pi.toc());
...
bool Xfeat::infer_origin(const cv::Mat &image_, float *heatmap,
                         float *descriptors) {
  if (!context_) {
    context_ = TensorRTUniquePtr<nvinfer1::IExecutionContext>(
        engine_->createExecutionContext());
  ...
  auto *output_desc = static_cast<float *>(
      buffers.getHostBuffer(xfeat_config_.output_tensor_names[0]));
  auto *output_heatmap = static_cast<float *>(
      buffers.getHostBuffer(xfeat_config_.output_tensor_names[1]));
  ASSERT(descriptors != nullptr && heatmap != nullptr);
  memcpy(descriptors, output_desc,
         buffers.size(xfeat_config_.output_tensor_names[0]));
  memcpy(heatmap, output_heatmap,
         buffers.size(xfeat_config_.output_tensor_names[1]));
...
// nms based grid
std::vector<std::pair<int, cv::Point2f>> Xfeat::nms_process(
    const std::vector<cv::Point2f> &pts, const std::vector<int> &sorted_idx,
    float dist_thresh) {
...
void Xfeat::detect_point(const float *heat_map,
                         Eigen::Matrix<float, 67, Eigen::Dynamic> &features,
...
void Xfeat::extract_descriptors(
    const float *descriptors,
    Eigen::Matrix<float, 67, Eigen::Dynamic> &features, int h, int w, int s) {
  ...
  Eigen::Array<float, 1, Eigen::Dynamic> kpts_x_norm =
      features.block(1, 0, 1, features.cols()).array() * sx + bx;
  Eigen::Array<float, 1, Eigen::Dynamic> kpts_y_norm =
      features.block(2, 0, 1, features.cols()).array() * sy + by;
  ...
  Eigen::Matrix<float, Eigen::Dynamic, Eigen::Dynamic> descriptor_matrix =
      features.block(3, 0, features.rows() - 3, features.cols());
  descriptor_matrix.colwise().normalize();
  features.block(3, 0, features.rows() - 3, features.cols()) =
      descriptor_matrix;
}

bool Xfeat::keypoints_decoder(
    const float *scores, const float *descriptors,
    Eigen::Matrix<float, 67, Eigen::Dynamic> &features) {
  TicToc tic_dp;
  detect_point(scores, features, resized_height, resized_width,
               xfeat_config_.keypoint_threshold, xfeat_config_.remove_borders,
               xfeat_config_.max_keypoints);
  ROS_DEBUG("xp: detect_point cost: %lf ms", tic_dp.toc());
  TicToc tic_ed;
  extract_descriptors(descriptors, features, resized_height / 8,
                      resized_width / 8, 8);
  ROS_DEBUG("xp: extract_descriptors cost: %lf ms", tic_ed.toc());

  features.block(1, 0, 1, features.cols()) =
      features.block(1, 0, 1, features.cols()) * w_scale;  // recovery scale
  features.block(2, 0, 1, features.cols()) =
      features.block(2, 0, 1, features.cols()) * h_scale;
  return true;
}

bool Xfeat::process_output(const BufferManager &buffers,
                           Eigen::Matrix<float, 67, Eigen::Dynamic> &features) {
  auto *output_desc = static_cast<float *>(
      buffers.getHostBuffer(xfeat_config_.output_tensor_names[0]));
