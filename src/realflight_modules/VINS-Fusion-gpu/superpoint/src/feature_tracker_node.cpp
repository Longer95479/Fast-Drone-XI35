            ...
            if (!img0_buf.empty() && !img1_buf.empty()) {
                ...
                if (time0 < time1) {
                    ...
                    if (round(1.0 * process_counts / (cur_time - first_image_time)) <= FREQ) {
                        ...
                        if (abs(1.0 * process_counts / (cur_time - first_image_time) - FREQ) <
                            0.01 * FREQ) {
                            first_image_time = cur_time;
                            process_counts   = 0;
                        }
...
int main(int argc, char **argv) {
    ...
    ros::Subscriber sub_img0 =
        nh.subscribe(tracker.feature_tracker_config.image0_topic, 100, img0_callback);
    ros::Subscriber sub_img1 =
        nh.subscribe(tracker.feature_tracker_config.image1_topic, 100, img1_callback);
