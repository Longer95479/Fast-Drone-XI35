...
ros::Publisher pub_img, pub_match, pub_line_img, pub_xfeats;

FeatureTracker tracker;
LineFeatureTracker line_tracker;
VLADExtractor vlad_extractor;
...
void sync_process() {
  ...
  int extract_vlad_feat = tracker.feature_tracker_config.extract_vlad_feat;
  while (1) {
    double cur_time = 0;
    std_msgs::Header header;
    ...
    if (tracker.stereo_cam) {
      ...
      if (!img0_buf.empty() && !img1_buf.empty()) {
        ...
        if (time0 < time1) {
          ...
          if (round(1.0 * process_counts / (cur_time - first_image_time)) <=
              FREQ) {
            process_this_frame = true;
            if (abs(1.0 * process_counts / (cur_time - first_image_time) -
                    FREQ) < 0.01 * FREQ) {
              first_image_time = cur_time;
              process_counts = 0;
            }
      ...
      if (!image0.empty() && process_this_frame) {
        ...
        if (use_opticalflow)
          tracker_future = thread_pool.submit(
              std::bind(&FeatureTracker::track_img_use_opticalflow, &tracker,
                        cur_time, std::ref(image0), std::ref(image1)));
        // tracker.track_img_use_opticalflow(cur_time, image0, image1);
        else
          tracker_future = thread_pool.submit(
              std::bind(&FeatureTracker::track_img, &tracker, cur_time,
                        std::ref(image0), std::ref(image1)));
        // tracker.track_img(cur_time, image0, image1);
        // line track
        std::future<void> line_tracker_future;
        if (DETECT_LINE) {
          line_tracker_future = thread_pool.submit(
              std::bind(&LineFeatureTracker::readImage, &line_tracker, cur_time,
                        std::ref(image0)));
        }
        tracker_future.get();
        if (DETECT_LINE) {
        ...
        if (DETECT_LINE &&
            line_tracker.line_tracker_config.associate_pts_at_front) {
          line_tracker.calAssociaPtsForLines(
              tracker.cur_un_pts, tracker.cur_pts,
              line_tracker.curFrame->lineSpEpUndist,
              line_tracker.curFrame->lineAssociaPts);
          line_tracker.DrawLIneWithAssociaPts();
        }
        pub_this_frame = true;
        ROS_INFO("feature track stereo cost %f ms", tic_tk.toc());

        // extarct vlad feature at a aysnc thread
        if (extract_vlad_feat) {
          vlad_extractor.copyFeatsIn(cur_time, tracker.getCurDescMap(), 4800);
          std::future<void> vlad_extractor_future = thread_pool.submit(
              std::bind(&VLADExtractor::processAndPublish, &vlad_extractor));
        }
        // publish xfeat feature for loop
        std::future<void> extract_more_future =
            thread_pool.submit(std::bind(&FeatureTracker::etractLoopFeatsAndPub,
                                         &tracker, std::ref(pub_xfeats)));
      }
    } else {
      cv::Mat image;
      m_buf.lock();
    ...
    if (pub_this_frame) {
      ...
      sensor_msgs::ChannelFloat32 ch_desc[64];
      ...
      auto &pts_desc = tracker.prev_xdesc;
      for (int i = 0; i < tracker.cur_ids.size(); i++) {
        ...
        // add xfeat descriptor
        for (int j = 0; j < 64; j++) {
          ch_desc[j].values.push_back(pts_desc[i][j]);
        }
      }
      if (tracker.stereo_cam) {
        ...
        for (int i = 0; i < tracker.right_ids.size(); i++) {
          ...
          ch6.values.push_back(0);  // no use
          feature_type.values.push_back(0);
          // keep ch of descriptor(placeholder)
          for (int j = 0; j < 64; j++) {
            ch_desc[j].values.push_back(0);
          }
        }
      }
      // add line features
      if (DETECT_LINE) {
        ...
        auto &lbd_desc = line_tracker.curFrame->lbdDesc;
        for (int i = 0; i < line_id.size(); i++) {
          ...
          // add lbd descriptor
          auto *row_ptr = lbd_desc.ptr<uchar>(i);
          for (int j = 0; j < 64; j++) {
            if (j < 32) {
              ch_desc[j].values.push_back(row_ptr[j]);
            } else {
              ch_desc[j].values.push_back(0);  // placeholder
            }
          }
        }
      }
      ...
      // add descriptor channel
      for (int i = 0; i < 64; i++) {
        feature_points->channels.push_back(ch_desc[i]);
      }
      ...
      if (tracker.feature_tracker_config.show_track) {
        ...
        if (!track_res.empty()) pubTrackImage(track_res, header.stamp.toSec());
...
int main(int argc, char **argv) {
  ...
  nh.param<string>("/feature_tracker_node/lightglue_plugin_path",
                   lightglue_plugin_path, "");
  if (argc != 2) {
    printf(
  ...
  vlad_extractor.init(nh, config_file, model_path);
  tracker.feature_predictor.init(nh);
  // prewarm for network
  tracker.prewarmForTracker();
  vlad_extractor.prewarm();
  // thread pool
  thread_pool.init();
  // publisher
  pub_img =
      nh.advertise<sensor_msgs::PointCloud>("/feature_tracker/feature", 1000);
  pub_match =
      nh.advertise<sensor_msgs::Image>("/feature_tracker/feature_img", 1000);
  pub_line_img =
      nh.advertise<sensor_msgs::Image>("/feature_tracker/line_img", 1000);
  pub_xfeats =
      nh.advertise<sensor_msgs::PointCloud>("/feature_tracker/xfeats", 1000);
  // subscriber
  ros::Subscriber sub_img0 = nh.subscribe(
      tracker.feature_tracker_config.image0_topic, 100, img0_callback);
  ros::Subscriber sub_img1 = nh.subscribe(
      tracker.feature_tracker_config.image1_topic, 100, img1_callback);
  ros::Subscriber sub_zc;
  if (line_tracker.line_tracker_config.detect_vertical_at_front)
    sub_zc = nh.subscribe<sensor_msgs::PointCloud>(
        "/vins_fusion/world_z_in_camera", 100,
        std::bind(&LineFeatureTracker::zAxisInCameraCallback, &line_tracker,
                  std::placeholders::_1));
