...
void FeaturePredictor::init(ros::NodeHandle& nh) {
  sub_imu_ = nh.subscribe("/mavros/imu/data_raw", 100,
                          &FeaturePredictor::inputImuMeas, this);
  sub_extrinsic_ =
      nh.subscribe("/vins_fusion/extrinsic", 100,
                   &FeaturePredictor::updateExtrinsicAndBias, this);
}
...
void FeaturePredictor::setCameraParam(const camodocal::CameraPtr& camera_ptr,
  ...
  grid_flow_corr_.resize(kGridRows * kGridCols,
                         CellFlow{cv::Point2d{0, 0}, cv::Point2f{0, 0}, 0});
  max_x_ = img_width - 1;
  max_y_ = img_height - 1;
}
...
void FeaturePredictor::resetReferenceFeature(
  ...
  for (size_t i = 0; i < cur_pts.size(); i++) {
    ...
    pts_idx_to_cell_idx_[i] =
        static_cast<int>(cur_pts[i].y / cell_height_) * kGridCols +
...
bool FeaturePredictor::calcuReleRot(double cur_frame_ts) {
  ...
  R_cur_ref_.setIdentity();
  if (imu_que_.empty() ||
      imu_que_.front()->header.stamp.toSec() > ref_frame_ts_ + 0.5)
    return false;
  ...
  while (!imu_que_.empty() &&
         imu_que_.front()->header.stamp.toSec() < ref_frame_ts_) {
  ...
  if (imu_que_.front()->header.stamp.toSec() > ref_frame_ts_) {
    ...
    Eigen::Vector3d ref_left_gyro(ref_left_imu->angular_velocity.x,
                                  ref_left_imu->angular_velocity.y,
                                  ref_left_imu->angular_velocity.z);
    double ref_right_time = imu_que_.front()->header.stamp.toSec();
    Eigen::Vector3d ref_right_gyro(imu_que_.front()->angular_velocity.x,
                                   imu_que_.front()->angular_velocity.y,
                                   imu_que_.front()->angular_velocity.z);
    double coffe_1 =
        (ref_right_time - ref_frame_ts_) / (ref_right_time - ref_left_time);
    double coffe_2 =
        (ref_frame_ts_ - ref_left_time) / (ref_right_time - ref_left_time);
    Eigen::Vector3d mid_gyro =
        coffe_1 * ref_left_gyro + coffe_2 * ref_right_gyro;
    integ_que.emplace_back(ref_frame_ts_, (mid_gyro - cur_bias_gyro));
  }

  sensor_msgs::ImuConstPtr cur_left_imu = imu_que_.front();
  while (!imu_que_.empty() &&
         imu_que_.front()->header.stamp.toSec() < cur_frame_ts_) {
    Eigen::Vector3d gyro_temp(imu_que_.front()->angular_velocity.x,
                              imu_que_.front()->angular_velocity.y,
                              imu_que_.front()->angular_velocity.z);
  ...
  Eigen::Vector3d cur_left_gyro(cur_left_imu->angular_velocity.x,
                                cur_left_imu->angular_velocity.y,
                                cur_left_imu->angular_velocity.z);
  double cur_right_time = imu_que_.front()->header.stamp.toSec();
  Eigen::Vector3d cur_right_gyro(imu_que_.front()->angular_velocity.x,
                                 imu_que_.front()->angular_velocity.y,
                                 imu_que_.front()->angular_velocity.z);
  double coffe_1 =
      (cur_right_time - cur_frame_ts_) / (cur_right_time - cur_left_time);
  double coffe_2 =
      (cur_frame_ts_ - cur_left_time) / (cur_right_time - cur_left_time);
...
void FeaturePredictor::getPrediction(double cur_frame_ts,
  ...
  for (size_t i = 0; i < ref_pts_.size(); i++) {
    ...
    cv::Point2f cur_pts_corr =
        grid_flow_corr_[pts_idx_to_cell_idx_[i]].flow_avg;
    if (cur_pts_rot[2] < 1e-5) {
      ...
      cv::Point2f cur_pts_pred =
          cv::Point2f(cur_pts_rot_plane(0), cur_pts_rot_plane(1)) +
          cur_pts_corr;
...
void FeaturePredictor::update(const std::vector<cv::Point2f>& cur_pts,
...
void FeaturePredictor::updateExtrinsicAndBias(
