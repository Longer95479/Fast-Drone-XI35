...
void LineFeatureTracker::zAxisInCameraCallback(
    const sensor_msgs::PointCloudConstPtr &zc_msg) {
  mtx_z.lock();
  ...
  cur_z.normalize();
  mtx_z.unlock();
  if (!is_z_usable) is_z_usable = true;
}
...
bool LineFeatureTracker::inBorder(const KeyLine &line) {
  ...
  return (BORDER_SIZE <= start_x &&
          start_x < line_tracker_config.col - BORDER_SIZE &&
          BORDER_SIZE <= start_y &&
          start_y < line_tracker_config.row - BORDER_SIZE) ||
         (BORDER_SIZE <= end_x &&
          end_x < line_tracker_config.col - BORDER_SIZE &&
          BORDER_SIZE <= end_y &&
          end_y < line_tracker_config.row - BORDER_SIZE);
}
//返回z轴消失点在归一化坐标系中的坐标
...
//返回两条线的角度差(0~π/2)
double LineFeatureTracker::getTwoLinesAbsAngle(const KeyLine &line0,
                                               const KeyLine &line1) {
  Vector2d l0_vec(line0.getEndPoint().x - line0.getStartPoint().x,
                  line0.getEndPoint().y - line0.getStartPoint().y);
  Vector2d l1_vec(line1.getEndPoint().x - line1.getStartPoint().x,
                  line1.getEndPoint().y - line1.getStartPoint().y);
  l0_vec.normalize();
  l1_vec.normalize();
  return acos(fabs(l0_vec.dot(l1_vec)));
}
//归一化坐标系下
double LineFeatureTracker::getTwoLinesAbsAngle(const Vector4d &line0,
...
double LineFeatureTracker::getTwoLinesDistByP2L(const KeyLine &line0,
                                                const KeyLine &line1) {
...
//归一化坐标系下
double LineFeatureTracker::getTwoLinesDistByP2L(const Vector4d &line0,
                                                const Vector4d &line1) {
  double x0 = line0[0];
  double y0 = line0[1];
  double x1 = line0[2];
...
//返回两个线段端点之间的距离
double LineFeatureTracker::getTwoLinesDistByP2P(const KeyLine &line0,
                                                const KeyLine &line1) {
...
double LineFeatureTracker::getTwoLinesDistByMid(const KeyLine &line0,
                                                const KeyLine &line1) {
...
void LineFeatureTracker::readIntrinsicParameter() {
  ...
  m_camera =
      CameraFactory::instance()->generateCameraFromYamlFile(calib_file[0]);
  K_ = m_camera->initUndistortRectifyMap(undist_map1, undist_map2);
}

void LineFeatureTracker::undistortedLineEndPoints(
    const vector<KeyLine> &key_lsd, vector<Vector4d> &line_undist) {
  line_undist.clear();
  ...
  for (auto &line : key_lsd) {
    ...
    un_pts_s /= un_pts_s.z();
    un_pts_e /= un_pts_e.z();
    line_undist.emplace_back(un_pts_s.x(), un_pts_s.y(), un_pts_e.x(),
                             un_pts_e.y());
  }
}
...
void LineFeatureTracker::calCurTrackCnt() {
  ...
  if (prevFrame->trackCnt.empty() && !curFrame->lineID.empty()) {
    for (auto id : curFrame->lineID) curFrame->trackCnt[id] = 1;
  } else {
...
// NMS实现
vector<int> LineFeatureTracker::lineNMSProcess(
    const vector<KeyLine> &vecTracked, const vector<KeyLine> &vecNew) {
  ...
  for (auto it = indexes.begin(); it != indexes.end();) {
    ...
    for (auto &k_line : vecTracked) {
      ...
      if (diff_ang < line_tracker_config.two_line_ang_thresh &&
          (diff_dist < line_tracker_config.two_line_dist_thresh ||
           diff_dist_mid < line_tracker_config.two_line_dist_thresh)) {
        remove_it = true;
        remove_cnt++;
        break;
      }
  ...
  sort(indexes.begin(), indexes.end(), [&vecNew](const int i1, const int i2) {
    return distance(vecNew[i1].getStartPoint(), vecNew[i1].getEndPoint()) >
           distance(vecNew[i2].getStartPoint(), vecNew[i2].getEndPoint());
  });
  // nms for new lines
  remove_cnt = 0;
  vector<int> res;
...
//划分出垂直线段
vector<LineType> LineFeatureTracker::lineClassify(
    const vector<Vector4d> &key_lsd, const cv::Point2f &vp_z) {
  ...
  for (int i = 0; i < key_lsd.size(); i++) {
    ...
    diff_dist *= 460.0;
    if (diff_ang < line_tracker_config.vertical_judge_ang &&
        diff_dist < line_tracker_config.vertical_judge_dist)
      type_res[i] = VERTICAL;
  }
  return type_res;
}
...
void LineFeatureTracker::readImage(double _cur_time, const cv::Mat &_img) {
  ...
  if (prevFrame->keyLsd.size() > 0) {
    ...
    for (int i = 0; i < lsd_matches.size(); i++) {
      if (lsd_matches[i].distance < 30) {
        ...
        if ((serr.dot(serr) < 200 * 200) && (eerr.dot(eerr) < 200 * 200) &&
            abs(line1.angle - line2.angle) < 0.1)  // 线段在图像里不会跑得特别远
    ...
    if (line_tracker_config.enable_line_nms) {
      ...
      ROS_DEBUG(
          "Before NMS new lines: %d, after NMS lines: %d, remove lines %d",
          vecLineNew.size(), nms_res.size(),
          vecLineNew.size() - nms_res.size());
    } else {
...
void LineFeatureTracker::DrawLine() {
  ...
  for (int i = 0; i < curFrame->lineID.size(); i++) {
    ...
    cv::line(rgba_image, curFrame->keyLsd[i].getStartPoint(),
             curFrame->keyLsd[i].getEndPoint(),
...
void LineFeatureTracker::DrawLineWithType() {
  ...
  for (int i = 0; i < curFrame->lineID.size(); i++) {
    if (curFrame->lineType[i] == VERTICAL)
      cv::line(rgba_image, curFrame->keyLsd[i].getStartPoint(),
               curFrame->keyLsd[i].getEndPoint(), cv::Scalar(255, 0, 0),
               2);  // BGR
    else
      cv::line(rgba_image, curFrame->keyLsd[i].getStartPoint(),
               curFrame->keyLsd[i].getEndPoint(), cv::Scalar(0, 255, 0), 2);
  }
...
void LineFeatureTracker::DrawLIneWithAssociaPts() {
  ...
  for (int i = 0; i < curFrame->lineID.size(); i++) {
    cv::line(rgba_image, curFrame->keyLsd[i].getStartPoint(),
             curFrame->keyLsd[i].getEndPoint(), cv::Scalar(255, 0, 0), 2);
    for (int j = 0; j < curFrame->lineAssociaPts[i].size(); j++) {
      cv::circle(rgba_image, curFrame->lineAssociaPts[i][j], 2,
                 cv::Scalar(0, 0, 255), 2);
    }
  }
  cv::cvtColor(rgba_image, imTrack, cv::COLOR_BGRA2BGR);
...
cv::Mat LineFeatureTracker::getTrackImage() { return imTrack; }

// 3 / fx = 0.00769
void LineFeatureTracker::calAssociaPtsForLines(
    const vector<cv::Point2f> &cur_un_pts, const vector<cv::Point2f> &cur_pts,
    const vector<Vector4d> &cur_un_lines,
    vector<vector<cv::Point2f>> &associa_pts) {
  ...
  for (auto &line : cur_un_lines) {
    ...
    for (int i = 0; i < pts_l_dist.rows(); i++) {
      // ROS_DEBUG("calAssociaPtsForLines: cur pts is (%lf, %lf), line is (%lf,
      // %lf, %lf, %lf), pts_l_dist is %lf", cur_pts_mat(i, 0), cur_pts_mat(i,
      // 1), line(0), line(1), line(2), line(3), pts_l_dist(i));
    ...
    // ROS_DEBUG("calAssociaPtsForLines: line-%d was associated %d points",
    // line_idx, cnt);
    line_idx++;
  }
}
