...
class LineFeatureTracker {
 public:
  ...
  vector<int> lineNMSProcess(const vector<KeyLine> &vecTracked,
                             const vector<KeyLine> &vecNew);
  cv::Point2f getVpzFromZc();
  vector<LineType> lineClassify(const vector<Vector4d> &key_lsd,
                                const cv::Point2f &vp_z);
  void undistortedLineEndPoints(const vector<KeyLine> &key_lsd,
                                vector<Vector4d> &line_undist);
  void calCurTrackCnt();
  void calCurVelocity();
  void DrawLine();
  ...
  void calAssociaPtsForLines(const vector<cv::Point2f> &cur_un_pts,
