...
class FeatureTracker {
  public:
    ...
    void readConfigParameter(
        const string& config_file, const string& model_prefix_path, const string& plugin_path = "");
    vector<cv::Point2f> undistortedPts(vector<cv::Point2f>& pts, camodocal::CameraPtr cam);
    vector<cv::Point2f> ptsVelocity(
        vector<int>& cur_ids, vector<cv::Point2f>& cur_un_pts,
    ...
    void track_img_use_opticalflow(
    ...
    void extractDescriptors(const vector<cv::Point2f>& pts, aligned_vector<DescV>& descs);
    void extractSquareROIPtsDesc(
        const cv::Point2f& ori_pt, int half_len, vector<cv::Point2f>& pts,
        aligned_vector<DescV>& descs);
    void extractROIFeatsWithNMS(
        const cv::Point2f& ori_pt, int half_len, vector<cv::Point2f>& pts,
        aligned_vector<DescV>& descs);
    pair<int, float> matchSingleDesc(const DescV& target_desc, aligned_vector<DescV>& descs);
