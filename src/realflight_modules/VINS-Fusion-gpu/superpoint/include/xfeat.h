...
class Xfeat {
 public:
  ...
  bool infer(const cv::Mat &image_,
 ...
 private:
  ...
  void detect_point(const float *heat_map,
                    Eigen::Matrix<float, 67, Eigen::Dynamic> &features, int h,
                    int w, float threshold, int border, int top_k);
  void extract_descriptors(const float *descriptors,
                           Eigen::Matrix<float, 67, Eigen::Dynamic> &features,
                           int h, int w, int s);

  std::vector<std::pair<int, cv::Point2f>> nms_process(
