...
struct FeatureTrackerConfig {
  ...
  void load(const std::string &config_file) {
    ...
    extract_vlad_feat = fsSettings["vlad_extractor"]["use_netvlad"];
  }
...
struct LineTrackerConfig {
  ...
  void load(const std::string &config_file) {
    ...
    detect_vertical_at_front =
        fsSettings["line_config"]["detect_vertical_at_front"];
    ...
    associate_pts_at_front =
        fsSettings["line_config"]["associate_pts_at_front"];
  }
...
struct PLNetConfig {
  ...
  void setModelPrefixPath(std::string model_prefix_path) {
    if (model_prefix_path.back() != '/') model_prefix_path += '/';
...
struct tarckAssistArg {
  void load(const std::string &config_file) {
    ...
    ratio_test_threshold =
        fsSettings["xfeat_opticalflow"]["ratio_test_threshold"];
  }
...
struct PointMatcherConfig {
  ...
  void setPluginPath(std::string _plugin_path) { plugin_path = _plugin_path; }
};

struct NetVLADConfig {
  std::vector<std::string> input_tensor_names;
  std::vector<std::string> output_tensor_names;
  std::string onnx_file;
  std::string engine_file;
  ...
  void load(const std::string &config_file) {
    ...
    cv::FileStorage fsSettings(config_file, cv::FileStorage::READ);
    fsSettings["vlad_extractor"]["onnx_file"] >> onnx_file;
    fsSettings["vlad_extractor"]["engine_file"] >> engine_file;
    input_tensor_names.push_back("input");
    output_tensor_names.push_back("output");
  }
  void setModelPrefixPath(std::string model_prefix_path) {
    if (model_prefix_path.back() != '/') model_prefix_path += '/';
    onnx_file = model_prefix_path + onnx_file;
