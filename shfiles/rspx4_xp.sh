sudo chmod 777 /dev/ttyTHS0 & sleep 1;
roslaunch realsense2_camera rs_camera.launch & sleep 4;
roslaunch mavros px4.launch & sleep 4;
find ../ -type f -name 'fast_drone_250.yaml' -exec sed -i 's/use_external_front_end: 0/use_external_front_end: 1/1' {} \;
roslaunch superpoint superpoint_frontend.launch & sleep 4;
roslaunch vins fast_drone_250.launch & sleep 4;
wait;
